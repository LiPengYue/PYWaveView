...
- (void)setupWaceView {
    ...
    //颜色数组
    waveView.colorMutableArray = [NSMutableArray arrayWithObject:self.colorArray.firstObject];
    //北京颜色数组
...
- (void)setupAddColor{
    UIButton *button = [[UIButton alloc]init];
    button.center = CGPointMake(self.view.center.x, self.stopWaveViewMaxY + 100);
    button.bounds = CGRectMake(0, 0, 200, 50);
    [self.view addSubview:button];
    [button addTarget:self action:@selector(clickAddColorButton:) forControlEvents:UIControlEventTouchUpInside];
    [button setTitle:@"点击我添加颜色" forState:UIControlStateNormal];
    
    [button setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}

- (void)clickAddColorButton: (UIButton *)button {
    if (self.colorCount >= self.colorArray.count) {
        self.colorCount = 0;
    }
    [self.waveView.colorMutableArray addObject:self.colorArray[self.colorCount]];
    self.colorCount ++;
